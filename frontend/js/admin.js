...
function lignesCoursProgramme(cours, messageVide = 'Aucun cours.') {
  ...
  return cours.map(p => `<tr><td><span class="prog-code-admin">${p.code}</span></td><td>${p.nom}</td><td>${heuresUE(p.cmi)}</td><td>${heuresUE(p.td)}</td><td>${heuresUE(p.tp)}</td><td>${p.credits} cr.</td><td class="admin-actions-cell"><button class="btn-icone" onclick="ouvrirModalDupliquerCours(${p.id})" aria-label="Ajouter à une autre filière" title="Ajouter ce cours à une autre filière">${icone('plus')}</button><button class="btn-icone" onclick="modifierProgramme(${p.id})" aria-label="Modifier">${icone('crayon')}</button><button class="btn-icone danger" onclick="supprimerProgramme(${p.id})" aria-label="Supprimer">${icone('corbeille')}</button></td></tr>`).join('');
}
